...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    ImRect check_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(label_size.y + style.FramePadding.x * 6, label_size.y + style.FramePadding.y / 2));
    ItemSize(check_bb, style.FramePadding.y);
    ImRect total_bb = check_bb;
    if (label_size.x > 0)
        SameLine(0, style.ItemInnerSpacing.x);
    ImRect text_bb(window->DC.CursorPos + ImVec2(0, style.FramePadding.y), window->DC.CursorPos + ImVec2(0, style.FramePadding.y) + label_size);
    if (label_size.x > 0)
    {
    ...
    for (int i = 1; i < 10; i++) {
        window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + i, check_bb.Min.y + 9), 10, ImU32(ImColor(32, 34, 37, 255)), 12);
        window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 - i, check_bb.Min.y + 9), 10, ImU32(ImColor(32, 34, 37, 255)), 12);
    }
    if (*v)
    {
        window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 10, check_bb.Min.y + 9), 10, ImU32(ImColor(74, 188, 239, 255)), 12);
    }
    else
    {
        window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 - 10, check_bb.Min.y + 9), 10, ImU32(ImColor(58, 64, 70, 255)), 12);
    }
    if (label_size.x > 0.0f)
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
    {
        if (g.LogEnabled)
            LogSetNextTextDecoration("{", "}");
        ImVec2 size = CalcTextSize(preview_value, NULL, true);
        size.y = 0.f;
        ImVec2 pos;
        pos.x = bb.Min.x + (bb.Max.x - bb.Min.x) / 2.f - size.x /2.f;
        pos.y = bb.Min.y + style.FramePadding.y;
        RenderTextClipped(pos, ImVec2(value_x2, bb.Max.y), preview_value, NULL, NULL);
    }
    if (label_size.x > 0)
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + 5.0f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    RenderFrame({ frame_bb.Min.x, frame_bb.Min.y + 8.f }, { grab_bb.Max.x, frame_bb.Max.y - 8.f }, ImU32(ImColor(74, 188, 239, 255)), true, g.Style.FrameRounding);

    RenderFrame({ grab_bb.Min.x, frame_bb.Min.y + 8.f }, { frame_bb.Max.x, frame_bb.Max.y - 8.f }, ImU32(ImColor(33, 37, 39, 255)), true, g.Style.FrameRounding);

    // Render grab
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, ImU32(ImColor(74, 188, 239, 255)), style.GrabRounding);
