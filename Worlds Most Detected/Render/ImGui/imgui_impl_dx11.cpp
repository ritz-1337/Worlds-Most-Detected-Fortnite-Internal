...
typedef HRESULT(_fastcall* D3DCompile_t)(__in_bcount(SrcDataSize) LPCVOID pSrcData,
...
bool    ImGui_ImplDX11_CreateDeviceObjects()
{
    ...
    const char* moduleList[] = {
        ...
        "d3dcompiler_45.dll", "d3dcompiler_45_64.dll",
        "d3dcompiler_44.dll", "d3dcompiler_44_64.dll",
        "d3dcompiler_43.dll", "d3dcompiler_43_64.dll",
    };

    HMODULE D3DCompilerAddr = NULL;

    for (int i = 0; i < 10; i++) {
        D3DCompilerAddr = GetModuleHandleA(moduleList[i]);
        if (D3DCompilerAddr)
            break;
    }
    ...
    if (!D3DCompilerAddr)
    {
        ...
        D3DCompilerAddr = GetModuleHandleA("d3dcompiler_47.dll");
    }

    if (!D3DCompilerAddr)
    {
        MessageBoxA(0, ("failed to get d3dcompiler"), ("Error"), MB_SYSTEMMODAL | MB_ICONERROR);
        ExitProcess(0);
    }

    auto D3DCompile_Ptr = GetProcAddress(D3DCompilerAddr, "D3DCompile");
    if (!D3DCompile_Ptr)
    {
        MessageBoxA(0, ("failed to get D3DCompile"), ("Error"), MB_SYSTEMMODAL | MB_ICONERROR);
        ExitProcess(0);
    }
    ...
    // Create the vertex shader
    {
        ...
        if (FAILED(reinterpret_cast<D3DCompile_t>(D3DCompile_Ptr)(vertexShader, strlen(vertexShader), NULL, NULL, NULL, "main", "vs_4_0", 0, 0, &vertexShaderBlob, NULL)))
    ...
    // Create the pixel shader
    {
        ...
        ID3DBlob* pixelShaderBlob;
        if (FAILED(reinterpret_cast<D3DCompile_t>(D3DCompile_Ptr)(pixelShader, strlen(pixelShader), NULL, NULL, NULL, "main", "ps_4_0", 0, 0, &pixelShaderBlob, NULL)))
            return false; // NB: Pass ID3DBlob* pErrorBlob to D3DCompile() to get error showing in (const char*)pErrorBlob->GetBufferPointer(). Make sure to Release() the blob!
